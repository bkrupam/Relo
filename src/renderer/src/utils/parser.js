// Phase 2: local simulation — replaced by Gemini API in Phase 3
...
export function simulateParse(fullText) {
  // Split by "and" or "&" to support multiple reminders in one string
  const parts = fullText.split(/\b(?:and|&)\b/i).map(s => s.trim()).filter(Boolean)
